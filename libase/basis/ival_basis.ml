...
module BothWrap = struct
  ...
  module Binary_Lattice = struct
    ...
    let _pretty ~size fmt x =
      Format.fprintf fmt "{signed: %a; unsigned: %a}"
        Ival.pretty x.signed Ival.pretty x.unsigned
    ;;
    ...
    let includes a b = Integer_Lattice.includes a.signed b.signed || Integer_Lattice.includes a.unsigned b.unsigned
    ...
    let bottom = {signed = Ival.bottom; unsigned = Ival.bottom }
  ...
  module Binary_Forward = struct
    ...
    let bofbool =
      ...
      if size == 1 then
        match x with
        | Quadrivalent_basis.Bottom -> Binary_Lattice.bottom
  ...
  module Binary_Backward = struct
    ...
    let bisle ~size a b bool =
      ...
      let g comp1 comp2 = 
        ...
        let f signedx x =
          ...
          | None -> None
          | Some y -> Some {signed = y; unsigned = Ival.narrow x.unsigned @@ wrapu ~size y}
        in
      ...
      | B.Bottom -> (if a = Binary_Lattice.bottom then None else Some Binary_Lattice.bottom),
                    (if b = Binary_Lattice.bottom then None else Some Binary_Lattice.bottom)
      | B.Top -> None, None
    ;;
    ...
    let biule ~size a b bool =
      let g comp1 comp2 = 
        ...
        let f unsignedx x =
          ...
          | None -> None
          | Some y -> Some {unsigned = y; signed = Ival.narrow x.signed @@ wraps ~size y}
        in
    ...
    let beq ~size a b res = match res with
      ...
      | B.Bottom -> inter_refines ~size a Binary_Lattice.bottom,
                    inter_refines ~size b Binary_Lattice.bottom
      | B.Top -> None, None
    ;;

    let biadd ~size ~nsw ~nuw a b res =
      inter_refines ~size a (Binary_Forward.bisub ~nsw ~nuw ~size res b),
      inter_refines ~size b (Binary_Forward.bisub ~nsw ~nuw ~size res a);;


    let bisub ~size ~nsw ~nuw a b res =
      inter_refines ~size a (Binary_Forward.biadd ~nsw ~nuw ~size res b),
      inter_refines ~size b (Binary_Forward.bisub ~nsw ~nuw ~size a res);;
    ...
    let bimul ~size ~nsw ~nuw a b res =
      ...
      if Binary_Lattice.is_bottom res then None,None
