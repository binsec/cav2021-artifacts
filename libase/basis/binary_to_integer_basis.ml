...
struct
  ...
  module Binary_Forward = struct
    ...
    let bunknown ~size = IF.iunknown ()
    let biconst ~size i = IF.iconst i
    let assume ~size cond i = IF.assume cond i
    let valid ~size _ = Quadrivalent_basis.Top
    ...
    let bxor ~size a b = bunknown ~size
    let bor ~size a b = bunknown ~size
    let band ~size a b = bunknown ~size
    ...
    let bconcat ~size1 ~size2 _ _ = IF.iunknown ()
    ...
    let biadd ~size ~nsw ~nuw a b = IF.iadd a b
    let bisub ~size ~nsw ~nuw a b = IF.isub a b
  ...
  module Binary_Backward = struct
    ...
    let biadd ~size  ~nsw ~nuw = IB.iadd
