...
struct
  ...
  let (&&) = C.ar2 F.(&&)
  let (||) = C.ar2 F.(||)
  (* MAYBE: Assume should exist only on tuples too. *)
  let assume = C.ar2 F.assume
...
struct
  ...
  let assume ~size = C.ar2 (F.assume ~size)
  let biadd ~size ~nsw ~nuw = C.ar2 (F.biadd ~size ~nsw ~nuw)
  let bisub ~size ~nsw ~nuw = C.ar2 (F.bisub ~size ~nsw ~nuw)    
  ...
  let bunknown ~size = C.ar0 (F.bunknown ~size)
  let valid ~size = C.ar1 (F.valid ~size)
...
module Convert_Tuple_Forward
  (C:Conversion)
  (F:Tuple_Forward with module Arity := C.From_Arity) =
struct

end


(* module Convert_All_Forward *)
(*     (C:Conversion) *)
(*     (F:All_Forward with module Arity := C.From_Arity) = *)
(* struct *)
