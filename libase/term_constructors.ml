...
(* Constructors for the term. They are written here once for all, in
   ...
   several times. *)
type binder_depth = int
type time = binder_depth * int   (* A unique identifier for terms that need it.
...
= struct
  ...
  let hash (x,_) = x
  let to_string (i,s) = s (* ^ "#" ^ (string_of_int i) *)
end
...
| Valid of size * 't                 (* binary<size> -> bool *)
...
| Malloc of Malloc_id.t * int * 't  (* malloc_size,store -> store * address *)
