...
let type_from_non_gadt = function
    | Term_types.Integer -> Any_type Integer
    | Term_types.Boolean -> Any_type Boolean
    | Term_types.Binary (size) -> Any_type (Binary size)
    | _ -> assert false
...
type ('arg,'ret) term =
  ...
  | Biadd: {size:size;nsw:bool;nuw:bool} -> ((binary,binary) ar2,binary) term
  | Bisub: {size:size;nsw:bool;nuw:bool} -> ((binary,binary) ar2,binary) term
...
let hash: type a b. (a,b) term -> int = function
  ...
  | Ieq -> 16
  | Ile -> 17
  | Biadd{size;nsw;nuw} -> size + 10 + (if nsw then 128 else 0) + (if nuw then 256 else 0)
  | Bisub{size;nsw;nuw} -> size + 11 + (if nsw then 128 else 0) + (if nuw then 256 else 0)
  | Bimul{size;nsw;nuw} -> size + 12 + (if nsw then 128 else 0) + (if nuw then 256 else 0)
  | Biudiv(size) -> size + 13
  | Bisdiv(size) -> size + 14
  | Biumod(size) -> size + 15
  | Bismod(size) -> size + 16
  | Bshl{size;nsw;nuw} -> size + 17 + (if nsw then 128 else 0) + (if nuw then 256 else 0)
  | Bashr(size) -> size + 18
  | Blshr(size) -> size + 19
  | Band(size) -> size + 20
  | Bor(size) -> size + 21
  ...
  | Bisle(size) -> size + 24
  | Biule(size) -> size + 25
  | Bconcat(s1,s2) -> s1 * 31 + s2 + 26
  | Bextract{size;index;oldsize} -> size * 97 + index * 31 + oldsize + 27
  | Bsext(size) -> size + 28
  | Buext(size) -> size + 29
  | Bofbool(size) -> size + 30
  | Itimes k -> 64 + (Z.hash k)
  | Iconst k -> 73 + (Z.hash k)
  | Biconst(size,k) -> size * 31 + 30 + (Z.hash k)
;;
...
let equal: type a b c d. (a,b) term -> (c,d) term -> bool = fun a b ->
  ...
  | Ieq ,Ieq  -> true
  | Ile ,Ile  -> true
  | Biadd{size=s1;nsw=nsw1;nuw=nuw1} ,Biadd{size=s2;nsw=nsw2;nuw=nuw2}->
    s1 = s2 && nsw1 = nsw2 && nuw1 = nuw2
  | Bisub{size=s1;nsw=nsw1;nuw=nuw1} ,Bisub{size=s2;nsw=nsw2;nuw=nuw2}->
    s1 = s2 && nsw1 = nsw2 && nuw1 = nuw2                              
  ...
  | _, _ ->
    if (hash a) == (hash b)
    then Codex_log.warning "Bad hash collision or mistake";
    false
;;
...
module Build = struct
  ...
  module Binary = struct
    ...
    let biadd ~size ~nsw ~nuw = Biadd{size;nsw;nuw}
    let bisub ~size ~nsw ~nuw = Bisub{size;nsw;nuw}        
...
module Unused_Eval(T:sig
  ...
  let eval:type arg res. (arg,res) term -> (arg,res) T.t = function
    ...
    | Biadd{size;nsw;nuw} -> (* T.biadd ~size ~nsw ~nuw *) assert false
    | Bisub{size;nsw;nuw} -> (* T.bisub ~size ~nsw ~nuw *) assert false
...
module Unused_Eval_Forward(T:sig
  ...
  let eval:type arg res. (arg,res) term -> arg T.pack -> res T.t = function
    ...
    | Biadd{size;nsw;nuw} -> fun arg -> let (b1,b2) = T.extract2 arg in (* T.biadd ~size b1 b2 *) assert false
    | Bisub{size;nsw;nuw} -> fun arg -> let (b1,b2) = T.extract2 arg in (* T.bisub ~size b1 b2 *) assert false
