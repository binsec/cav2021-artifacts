...
module Term_Kind_Hash = struct
  ...
  let hash_subterms = function
    ...
    | Bshl(_,_,_,x,y) | Bashr(_,x,y) | Blshr(_,x,y) | Free(x,y) -> hash_pair x.id y.id
    ...
    | Malloc(id,size,mem) -> hash_triple (Malloc_id.hash id) size mem.id
