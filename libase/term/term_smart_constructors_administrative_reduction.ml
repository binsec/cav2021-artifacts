...
module Make(O:Term_smart_constructors_sig.S):Term_smart_constructors_sig.S = struct
  ...
  module Boolean = struct
    ...
    (* TODO: Transform assume false into assume(false,bottom) *)
    let assume cond x =
      match cond.kind with
      | True -> x
      | _ -> O.Boolean.assume cond x
    
    let bottom = O.Boolean.bottom
    type boolean = O.Boolean.boolean

    let unknown = O.Boolean.unknown
  end 
  ...
  module Binary = struct
    ...
    let valid ~size bin = match bin.kind with
      | Tuple_get({kind=Malloc _},0) -> O.Boolean.true_
      | _ -> O.Binary.valid ~size bin
    ;;
    ...
    (* TODO: Transform assume false into assume(false,bottom) *)
    let assume ~size cond x =
      match cond.kind with
      | True -> x
      | _ -> O.Binary.assume ~size cond x
