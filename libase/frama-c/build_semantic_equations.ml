...
module Make(L:Lang)(Conv:Conv with module L = L) = struct
  ...
  module M' = struct
    ...
    module Monadic_Arity:sig
      ...
      val bunknown: size:int -> L.binary m

      val get_mem: L.memory m
      (* MAYBE: do [assume_mem] simultaneously with add_alarm? *)
      val assume_mem: L.boolean -> L.memory m
      ...
      let assume_mem bool = fun state -> match state.mem with
        | None -> assert false
        | Some mem -> 
           L.Monadic_Arity.bind (L.Memory.assume bool mem) (fun mem ->
             L.Monadic_Arity.return (mem,{mem = Some mem}))
      ;;
      let get_mem state = match state.mem with
        | None -> assert false
        | Some v -> L.Monadic_Arity.return (v,state)


      let register_alarm_with_assume (alarm,loc) bool = fun st ->
        L.Monadic_Arity.bind (Conv.register_alarm alarm loc bool) @@ fun () ->
        let mem = Extlib.the st.mem in        
        L.Monadic_Arity.bind (L.Memory.assume bool mem) @@ fun mem ->
      ...
      let bunknown ~size = return2 (L.Binary.bunknown ~size)
    ...
    module Binary:Transfer_functions.Binary_Forward with module Arity := Monadic_Arity
      ...
      Transfer_functions_conversions.Convert_Binary_Forward(Conversion)(L.Binary)

    module Boolean:Transfer_functions.Boolean_Forward with module Arity := Monadic_Arity
                                                       and type boolean = L.boolean =
      Transfer_functions_conversions.Convert_Boolean_Forward(Conversion)(L.Boolean)
        
    module Memory:Transfer_functions.Memory_Forward with module Arity := Monadic_Arity
                                                    and type binary = L.binary
                                                    and type boolean = L.boolean
                                                    and type memory = L.memory =
      Transfer_functions_conversions.Convert_Memory_Forward(Conversion)(L.Memory)      
  ...
  let constant env ~size = function
    ...
    | CReal _ -> M.Monadic_Arity.bunknown ~size
  ;;
  ...
  let rec expression' env exp =
    ...
    | Lval(lv) ->
      M.Monadic_Arity.get_mem >>= fun mem ->
      lvalue env lv >>= fun loc ->
      let size = loc.size in
      let address = loc.address in
      M.Binary.valid ~size address >>= fun valid ->
      ...
      then M.Monadic_Arity.bunknown ~size
      else M.Memory.load size mem address) >>= fun v -> begin
    ...
    | StartOf lv -> lvalue env lv >>= fun loc ->
      ...
      let (TArray(t,_,_,_)) = Cil.unrollType @@ Cil.typeOfLval lv in
    ...
    | CastE(to_typ,subexp) ->
       ...
       | TFloat _, TFloat _ -> M.Monadic_Arity.bunknown ~size:(Cil.bitsSizeOf to_typ)
       ...
       | _, _ -> Kernel.fatal "cast not handled: from %a to %a" 
  ...
  and binop env exp_size bop e1 e2 =
    ...
    let apply bop = match bop with
      ...
      | PlusA | MinusA | Mult | Div | Mod when Cil.isFloatingType typ_e1 ->
        ...
        fun _v1 _v2 -> M.Monadic_Arity.bunknown ~size:exp_size

      (* Arithmetical operations. *)
      | PlusA -> M.Binary.biadd ~size:exp_size ~nsw ~nuw
      | MinusA -> M.Binary.bisub ~size:exp_size ~nsw ~nuw
      ...
      | MinusPI -> fun v1 v2 -> 
        ...
        M.Binary.bisub ~size:ptr_bit_size ~nsw:false ~nuw:false zero off >>= fun moff ->
        M.Binary.bindex ~size:ptr_bit_size k v1 moff
      | MinusPP -> fun v1 v2 ->
        let k = Cil.(bytesSizeOf (typeOf_pointed (typeOf e1))) in
        let k = Z.of_int k in
        M.Binary.bisub ~size:exp_size ~nsw:false ~nuw:false v1 v2 >>= fun diff ->
  ...
  and unop env exp_size uop e1 =
    ...
    | Neg ->
       ...
       M.Binary.bisub ~size:exp_size ~nsw:true ~nuw:false zero v1
  ...
  and loffset env typ (v:L.binary) offs =
    ...
    | Index(exp,offs) ->
       ...
       (match Cil.unrollType typ with
        | TArray(_elt_typ,length,_elt_typ_bitssize,_) -> begin
            ...
            | Some length, true ->
              ...
              let length = z_of_integer @@ Extlib.the (Cil.constFoldToInt ~machdep:true length) in
              M.Binary.biconst ~size:ptr_bit_size Z.zero >>= fun zero ->
  ...
  let the_mem state = Extlib.the state.mem
  ...
  let store_lvalue ~instr_loc env lv newv newvsize mem =
    ...
    lvalue env lv >>= fun loc ->
    M.Binary.valid ~size:loc.size loc.address >>= fun valid ->
    ...
    | Some bitfield ->
      ...
      L.Memory.load loc.size mem loc.address >>= fun oldv ->
  ...
  let call_to_unknown env mem ret f args instr_loc =
    ...
    (match ret with
     ...
     | Some lv ->
       ...
       L.Binary.bunknown ~size >>= fun value ->
    ...
    | Set(lvdst,exp,instr_loc) when false && Cil.isStructOrUnionType @@ Cil.unrollType @@ Cil.typeOf exp ->
      ...
      let rec f lvdst lvsrc mem =
        ...
        if Cil.isArithmeticOrPointerType typ then
          ...
          let len = match typ with TArray(_,i,_,_) -> i | _ -> assert false in
          ...
          let compinfo = match typ with Cil_types.TComp(ci,_,_) -> ci | _ -> assert false in
