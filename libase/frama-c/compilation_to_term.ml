...
module Lang = struct
  ...
  module Binary = struct
    ...
    let bunknown ~size = (SmartCons.Binary.bunknown_explicit ~size ~level:!loop_nesting);;
  end
  ...
  let varinfo_id =
    ...
    fun vi ->
      ...
      with Not_found ->
        let id =
          Term_constructors.Malloc_id.fresh (Format.asprintf "%a" (* Varinfo_Enclosing_Function *)Cil_datatype.Varinfo.pretty vi) in
        Cil_datatype.Varinfo.Hashtbl.replace varinfo_id_hash vi id;
        id
  ;;

  (* Allocate a variable, and fill it with some data.
     TODO: Filling is not necessary if it is dead. *)
  let allocate_var mem vi initf =
    ...
    let addr,mem = try
      ...
      with Function_Type -> Binary.bunknown ~size:ptr_size, Memory.Some_Memory mem
    in
  ...
  let compile_block_entry mem block =
    ...
    in
    List.fold_left (fun (mem) vi -> allocate_var mem vi to_store) mem locals
  ;;
...
let compile_function_entry kf initial_mem arg =
  ...
  let (memory,_) = List.fold_left (fun (acc,i) vi ->
      let acc = Lang.allocate_var acc vi (fun ~size -> Array.get arg i)  in
  ...
  let module Transfer_functions_inline = struct
    ...
    let malloc ret f args instr_loc stmt input_mem =
      ...
      let malloc_size = match Cil.constFoldToInt ~machdep:true sizeexp with
        ...
        | None -> assert false
      in
      let id = Term_constructors.Malloc_id.fresh ("malloc" ^ string_of_int stmt.sid) in
...
let write_initial_state ~libentry mem globals_used = 
  ...
  let do_global var (mem,allocated_vars,initialized_vars) =
    let libentry = libentry || (var.vstorage == Extern) in
    ...
    let initial_valuef,initialized_vars = match init with
      | None ->
        let initialized_vars = VarSet.add var initialized_vars in
        if libentry then init_unknown, initialized_vars
        else (fun ~size -> Binary.biconst ~size Z.zero), initialized_vars
      | Some(SingleInit e) -> begin
          if libentry && (Cil.typeHasQualifier "volatile" var.vtype ||
                          not (Cil.typeHasAttribute "const" var.vtype))
          then init_unknown, VarSet.add var initialized_vars
          else
            match Cil.constFoldToInt ~machdep:true e with
            | Some i ->
              (fun ~size -> Binary.biconst ~size (Translator.z_of_integer i)),
              VarSet.add var initialized_vars
            | None -> init_unknown, initialized_vars
        end
      | _ -> init_unknown, initialized_vars
    in
  ...
  let initialize_var var mem =
    ...
    let rec doit lv init typ mem =
      ...
      match init with
      | SingleInit e when libentry && (let base_typ = Cil.typeOfLval lv in
                                      Cil.typeHasQualifier "volatile" base_typ ||
                                      not (Cil.typeHasAttribute "const" base_typ)) -> (mem)
      | SingleInit(e) ->
...
let initialize_strings strings_used mem =
  ...
  Datatype.String.Set.fold (fun str mem ->
      ...
      let size = 8 * malloc_size in
      let id = Term_constructors.Malloc_id.fresh str in
...
let initialize_function_ptrs functions_used mem =
  ...
  Cil_datatype.Varinfo.Set.fold (fun f mem ->
      Lang.allocate_var mem f (fun ~size -> Lang.Binary.bunknown_explicit ~size ~level:0)
    ) functions_used mem
;;
