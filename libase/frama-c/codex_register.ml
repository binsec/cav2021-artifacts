    ...
    try
      ...
      let lattice = Compilation_to_term.Callstack_Trie.fold exptrie
          (fun (x,term) acc -> match x with
             ...
             | `Binary -> Analysis.Query.Binary_Lattice.join ~size acc @@ recover_basis term) Analysis.Query.Binary_Lattice.bottom in
...
module Main_Domain() = struct
  ...
  module NonRel_Domain = Domains_constraints_nonrelational.Make(Constraints)(Basis.Ival)
  (* module Constraint_Domain = Constraint_Domain'''.Base_Domain *)

  module Propag_Domain = Domains_constraints_constraint_propagation.Make(Constraints)(Basis.Ival)
  ...
  (* let module IntegerD = Constraint_domain.Make_Integer(Basis.Ival) *)
  ...
  module BinaryD2 = Constraint_domain2.Make(Constraints)(Domains_constraints_constraint_propagation.Make(Constraints)(Basis.Ival))
 
  (* module Param = Constraint_domain.Make_Binary(Basis.Ival) *)
  (* module Param = Constraint_domain.Make_Binary(Binary_to_integer_basis.Lift(Basis.Ival)) *)
