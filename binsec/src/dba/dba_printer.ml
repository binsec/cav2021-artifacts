...
module Make(R:Renderer) : DbaPrinter = struct
  ...
  let pp_instruction n ppf instruction =
    ...
    | Dba.Instr.Nondet (lhs, tag, id) ->
      let control_tag = match Dba.Non_deterministic_tag.controlled tag with
          | Dba.Non_deterministic_tag.Uncontrolled -> ""
          | Dba.Non_deterministic_tag.Controlled -> "controlled"
          | Dba.Non_deterministic_tag.Parameter -> "forall"
      in
       if control_tag <> "" then
         fprintf ppf "%a := %s%a"
           pp_lhs lhs
         ...
         fprintf ppf "%a := nondet(%a)%a"
           pp_lhs lhs
           pp_region (Dba.Non_deterministic_tag.region tag)
           suffix id
