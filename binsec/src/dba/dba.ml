...
module Non_deterministic_tag = struct
  type control = Uncontrolled | Controlled | Parameter
  type t = {
      region : region;
      controlled : control;
      name: string option; (* name hint *)
    }

  (* The invariant below makes it symmetric with the concrete syntax defined in
     [parser.mly].

     Since there's no way to create a controlled value located elsewhere than
     the region of constants in the concrete syntax, we enforce this invariant
     here as well for consistency. There is no fundamental reason why a
     controlled value could not be found in another region.
   *)
  let create ?name ?(region=`Constant) ?(controlled=Uncontrolled) () =
    assert (controlled <> Controlled || (controlled = Controlled && region = `Constant));
    { region; controlled; name }
  ...
  let pp_control ppf c = (match c with
...
= struct
  ...
  let ite condition then_expr else_expr =
    ...
    | Cst (`Constant, b) when Bitvector.is_zero b ->
      else_expr
    | Cst (`Constant, b) when Bitvector.is_one b ->
      then_expr
    | _ ->
  ...
  module Straight = struct
    ...
    let symmetric_binary op e1 e2 =
      let s1 = size_of e1 in
      let s2 = size_of e2 in
      if (s1 <> s2) then
        Kernel_options.Logger.fatal "Dba illegal binary operation between bitvectors of size %d and %d" s1 s2
      else
        binary op e1 e2
  ...
  *)
  ...
  and logand e1 e2 = match e1, e2 with
    ...
    | Cst (`Constant, b1), Binary (Binary_op.Concat, e3, e4) ->
...
(*    Format.printf "store : %d@." (Expr.size_of e); *)
    let ws = Kernel_options.Machine.word_size () in
    let expr_size = Expr.size_of e in
    if (ws != expr_size) then
      Kernel_options.Logger.fatal "DBA store index of size %i instead of the machine word size %i" expr_size ws
    else
...
module Instr = struct
  ...
  type t =
    ...
    | Nondet of LValue.t * Non_deterministic_tag.t * id
  ...
  let non_deterministic ?tag lv nid =
    let tag =
      match tag with
      | None -> Non_deterministic_tag.create ()
      | Some t -> t
    in Nondet (lv, tag, nid)
