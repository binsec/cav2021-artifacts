...
module Caddress = struct
  ...
  let create base id =
    if id < 0 then
      Logger.fatal "attempted to create invalid code address %a %d" Virtual_address.pp base id;
    { base; id; }
...
module Instruction = struct
  ...
  let set_successor i id =
    ...
    | Nondet (lv, tag, _) -> non_deterministic lv ~tag id
  ...
  let reloc =
    ...
    fun ?(outer=fun x -> x) ?(inner=fun x -> x) -> function
      ...
      | Nondet (lv, tag, id) -> non_deterministic lv ~tag (inner id)
  ...
  let generic_reset_successors ~p ~f instr =
    ...
    | Nondet (lv, tag, id) -> non_deterministic lv ~tag (new_id id)
