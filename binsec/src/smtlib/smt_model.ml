...
let is_non_boolean_bitvector sort = match get_bitvector_size sort with
...
let is_non_bl_bv_non_bl_bv_function args sort =
  match args with
  | [arg] ->
    let arg_sort = sort_of_sorted_var arg in
    is_non_boolean_bitvector arg_sort && is_non_boolean_bitvector sort
  | [] | _ -> false


let is_non_bl_bv_non_bl_bv_array sort =
  match sort.sort_desc with
  ...
  | SortFun (id, sorts) ->
    ...
    | IdSymbol symbol, s1 :: [s2] ->
      if String.compare (get_symbol_name symbol) "Array" = 0 then
        is_non_boolean_bitvector s1 && is_non_boolean_bitvector s2
      else false
  ...
  | CmdDefineFun fdef ->
    begin
      ...
      | FunDef (_, _, args, sort, _) ->
        is_non_bl_bv_non_bl_bv_array sort
        ||
        is_non_bl_bv_non_bl_bv_function args sort
    end
  | _ -> false
...
let get_indirection_name term =
  ...
  | _ -> Kernel_options.Logger.fatal "unexpected term in model:@ %a" Smtlib_pp.pp_term term
;;
...
let find_and_add_memory smt_model functions =
  ...
  | [f] ->
    ...
    if is_ite_memory body then memory_from_ite smt_model body
      ...
      Logger.warning "No extractor for this memory representation:@.%a" Smtlib_pp.pp_commands functions
  | f :: tail ->
    ...
    if is_ite_memory fbody then memory_from_ite smt_model fbody
      ...
      if is_ite_memory gbody then memory_from_ite smt_model gbody
        ...
        Logger.warning "Unexpected memory encoding from SMT model@.%a" Smtlib_pp.pp_commands functions
