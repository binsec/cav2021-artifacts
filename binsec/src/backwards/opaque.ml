...
module Translator = struct
  ...
  let add_comment pp v t =
      let cmt_str = pp Format.str_formatter v;
    ...
    let undef lv t =
      ...
      | Var { Dba.name; Dba.size = bitsize; _} ->
         begin
           ...
           let state = Sse_symbolic.State.declare  ~controlled:Dba.Non_deterministic_tag.Uncontrolled name sort t.state in
           let nodes = get_succs t in
           match nodes with
           | [hidx] -> Continue { t with hidx; state; }
           | [] -> assert false
...
let init_mem ~filename symb_st =
  if not (Sys.file_exists filename) then symb_st
    ...
    let f syst init =
      ...
      | Instruction i -> Logger.fatal "Unimplemented initialize with free dba instruction %a" Dba_printer.Ascii.pp_instruction i
      | Assignment (lval, _, _) ->
         let msg =
         ...
         failwith msg
      | Mem_load(addr, size) ->
        Sse_symbolic.State.assign_from_image syst ~addr ~size
