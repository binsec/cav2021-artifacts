...
let compute addr instr m conds djmps pp_conds glbs:
  ...
  | Dba.Instr.Nondet (lhs, tag, id_suiv) ->
    let m, glbs =
      let region = Dba.Non_deterministic_tag.region tag in
      match region with
      | `Malloc _ ->
        let f_aux r x = (
          match r with
          | `Malloc _ -> x = Dba.Freeable
        ...
        in
        let v =
          Region_bitvector.non_deterministic r (Dba_types.LValue.unsafe_bitsize lhs) in
        update_memory lhs v m conds glbs
      | `Constant
      | `Stack ->
        let v =
          Region_bitvector.non_deterministic region (Dba_types.LValue.unsafe_bitsize lhs) in
        update_memory lhs v m conds glbs
    in
    let a = Dba_types.Caddress.reid addr id_suiv in
    let next = Some (check_exec_permission a m conds glbs) in
