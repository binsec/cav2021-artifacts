...
let rec and_then: 'a. 'a Seq.t list -> 'a Seq.t = fun l () ->
  let open Seq in
  match l with
  | [] -> Nil
  | [x] -> x()
...
let rec flatten_seq : 'a. 'a Seq.t Seq.t -> 'a Seq.t = fun s () ->
  let open Seq in
  match s () with
  | Nil -> Nil
  | Cons(x, tl) -> match x () with
    | Nil -> (flatten_seq tl) ()
...
struct
  ...
  let rec dto_seq_forward f =
    let open Seq in fun () -> match f with
    | Zero -> Nil
  ...
  let rec qto_seq_forward : type b. b queue -> b Seq.t = function
    ...
    | Deep (f, m, r) ->
      ...
      let seq_of_seqs = Seq.map
          (fun (a,b) () -> Seq.(Cons(b, return a)))
          seq_of_pairs in
      and_then [
        (dto_seq_forward r);
        (flatten_seq seq_of_seqs);
        (dto_seq_forward f)
      ]
