...
module Generic_make(D: DECL) = struct
  ...
  module Builder = struct
    ...
    (** maps "x" to "no-x" and "no-x" to "x" *)
    let inverse_name name = if
      String_utils.starts_with ~substring:"no-" name then
    ...
    module Boolean(P : sig include CLI_DECL val default: bool end) =
      struct
        ...
        let reset_doc = Printf.sprintf "converse of %s" P.name

        let spec = Unit (fun () -> set (not P.default))
        let reset_spec = Unit (fun () -> set (P.default))
        
        let key = P.name
        let reset_key = inverse_name P.name

        let cspec = Cli_spec.simple ~key ~doc ~spec
        let reset_cspec = Cli_spec.simple ~key:reset_key ~doc:reset_doc ~spec:reset_spec

        let is_default () = !value = P.default

        let _ = extend cspec
        let _ = extend reset_cspec
      end
