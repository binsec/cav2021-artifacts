...
type bv_unop =
  ...
  | BvNeg
  | BvRepeat of int
  | BvZeroExtend of int (** add [i] zero bits *)
  | BvSignExtend of int (** add [i] sign bits *)
  | BvRotateLeft of int
...
and ax_term_desc =
  ...
  | Store  of int * ax_term * bv_term * bv_term
          (** size of store, array, index, value *)
...
module Printing = struct
  ...
  and p_bvterm fmt bv =
    ...
    | BvCst c ->
      fprintf fmt "(0x%x %i)"
        (Bigint.int_of_big_int (Bitvector.value_of c))
        (Bitvector.size_of c)
    | BvFun ({ bv_name ; _ }, []) ->
...
let bl_var_compare a b = compare a.bl_name b.bl_name

let bv_var bv_name bv_size =
  let bv_hash = Hashtbl.hash (bv_name, bv_size) in
  { bv_hash; bv_name; bv_size }

let bv_var_compare a b = compare (a.bv_name, a.bv_size) (b.bv_name, b.bv_size)
...
let ax_var_compare a b = compare (a.ax_name, a.idx_size, a.elt_size) (b.ax_name, b.idx_size, b.elt_size)
...
let naive_mk_bv_cst bv bv_term_size =
  let bv_term_hash = -(Hashtbl.hash bv) in
  let bv_term_desc = BvCst bv in
  { bv_term_hash; bv_term_desc; bv_term_size }

let bv_cst_interning = Basic_types.Int.Htbl.create 1237

let mk_bv_cst bv =
  let size = Bitvector.size_of bv in
  if size <= 32 then
    let index = (Bitvector.to_int bv) + (size lsl 33) in
    try
      Basic_types.Int.Htbl.find bv_cst_interning index
    with Not_found ->
      begin
        let res = naive_mk_bv_cst bv size in
        Basic_types.Int.Htbl.add bv_cst_interning index res;
        res
      end
  else
    naive_mk_bv_cst bv size
...
let bv_term_raw bv_term_desc =
  ...
  | BvIte (bl,bv1,bv2) ->
    ...
    let bv_term_hash = Hashtbl.hash (bl.bl_term_hash, bv1.bv_term_hash, bv2.bv_term_hash) in
    let bv_term_size = bv1.bv_term_size in
    { bv_term_hash; bv_term_desc; bv_term_size }

  | Select (n,ax,bv) ->
    if n < 0 then Formula_options.Logger.fatal "tried to create a select of negative length: select %i %a %a" n Printing.p_axterm ax Printing.p_bvterm bv;
    if ax.idx_term_size <> bv.bv_term_size then
      Formula_options.Logger.fatal "select %i %a %a: expected array index size (%i) is different than given index bv size (%i)"
        n Printing.p_axterm ax Printing.p_bvterm bv
        ax.idx_term_size bv.bv_term_size;
...
let rec bv_term bv_term_desc =
  match bv_term_desc with
  | BvCst _ | BvFun _ -> bv_term_raw bv_term_desc

  | BvLet ([],bv) -> bv
  | BvLet _ -> bv_term_raw bv_term_desc
    ...
    (match is_bv_cst bv with
     | None ->
       (match u with
        ...
        | BvExtract i ->
          if i.Interval.lo < 0 || i.Interval.hi >= bv.bv_term_size then
            let msg = Format.asprintf
                "[%d, %d] is an invalid extraction for %a (not included in [0, %d])"
                i.Interval.lo i.Interval.hi
                Printing.p_bvterm bv
                bv.bv_term_size
            in
            raise (Invalid_argument msg)
          else if i.Interval.lo = 0 && i.Interval.hi = bv.bv_term_size - 1 then bv
          else
            match bv.bv_term_desc with
    ...
    (match is_bv_cst bv1, is_bv_cst bv2 with
       ...
       (match b with
          ...
          if equal_bv_term bv1 bv2 then mk_bv_zeros bv_term_size
            ...
            (match bv1.bv_term_desc, bv2.bv_term_desc with
             ...
             | BvBnop (BvSub, bv11, bv12), BvBnop (BvAdd, bv21, bv22) ->
               ...
               | None, None, None, Some _ ->
                 mk_bv_sub (mk_bv_sub bv1 bv21) bv22
               | _, _, _, _ -> bv_term_raw bv_term_desc)
        ...
        | BvConcat ->
          (match bv1.bv_term_desc, bv2.bv_term_desc with
           ...
           | Select (n1, ax1, idx1), Select (n2, ax2, idx2) when equal_ax_term ax1 ax2 ->
     ...
     | None, Some bv ->
       (match b with
        ...
        | BvConcat | BvCmp
        | BvUrem | BvSrem | BvSmod -> bv_term_raw bv_term_desc)
    ...
    (match is_bl_cst bl with
     ...
     | None ->
       if equal_bv_term bv1 bv2 then bv1
         ...
         match is_bv_cst bv1, is_bv_cst bv2 with
         | None, None | None, Some _ | Some _, None -> bv_term_raw bv_term_desc
         | Some b1, Some b2 ->
  ...
  | Select _ -> bv_term_raw bv_term_desc
...
let bv_fun_interning = Basic_types.String.Htbl.create 1237

let naive_mk_bv_fun fn lst = bv_term (BvFun (fn,lst))
let mk_bv_fun fn lst     = match lst with
  | [] -> begin 
      try
        let res = Basic_types.String.Htbl.find  bv_fun_interning fn.bv_name in
        if res.bv_term_size != fn.bv_size then naive_mk_bv_fun fn lst
        else res
    with Not_found ->
      begin
        let res = naive_mk_bv_fun fn lst in
        Basic_types.String.Htbl.add bv_fun_interning fn.bv_name res;
        res
      end
  end
  | _ -> naive_mk_bv_fun fn lst
...
let mk_bv_extract      i bv =
  ...
  then Formula_options.Logger.fatal "invalid extract of interval [%i;%i[ for bv %a of size %i"
      i.lo
      i.hi
      Printing.p_bvterm bv
      bv.bv_term_size
...
module BlVarMap = Map.Make
    (struct
      type t = bl_var
      let compare = bl_var_compare
    end)

module BvVarMap = Map.Make
    (struct
      type t = bv_var
      let compare = bv_var_compare
    end)

module AxVarMap = Map.Make
    (struct
      type t = ax_var
      let compare = ax_var_compare
    end)
...
module BlTermHashamt = Hashamt.Make
    (struct
      type t = bl_term
      let equal bl1 bl2 = bl1 = bl2
      let hash bl = bl.bl_term_hash
...
module BvTermHashamt = Hashamt.Make
    (struct
      type t = bv_term
      let equal bv1 bv2 = bv1 = bv2
      let hash bv = bv.bv_term_hash
...
module AxTermHashamt = Hashamt.Make
    (struct
      type t = ax_term
      let equal ax1 ax2 = ax1 = ax2
      let hash ax = ax.ax_term_hash
