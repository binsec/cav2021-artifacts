...
(** a map designed with a variable as key *)
module BindMap :
sig

  type ('bl, 'bv, 'ax) t

  val empty: ('bl, 'bv, 'ax) t

  (** retrieves the value for this variable, or none *)
  val bl_lookup : ('bl, 'bv, 'ax) t -> bl_var -> 'bl option

  (** retrieves the value for this variable, or none *)
  val bv_lookup : ('bl, 'bv, 'ax) t -> bv_var -> 'bv option

  (** retrieves the value for this variable, or none *)
  val ax_lookup : ('bl, 'bv, 'ax) t -> ax_var -> 'ax option

  (** store a value with this variable as key *)
  val bl_store : ('bl, 'bv, 'ax) t -> bl_var -> 'bl -> ('bl, 'bv, 'ax) t

  (** store a value with this variable as key *)
  val bv_store : ('bl, 'bv, 'ax) t -> bv_var -> 'bv -> ('bl, 'bv, 'ax) t

  (** store a value with this variable as key *)
  val ax_store : ('bl, 'bv, 'ax) t -> ax_var -> 'ax -> ('bl, 'bv, 'ax) t

  (** removes the binding for this variable *)
  val bl_del : ('bl, 'bv, 'ax) t -> bl_var -> ('bl, 'bv, 'ax) t

  (** removes the binding for this variable *)
  val bv_del : ('bl, 'bv, 'ax) t -> bv_var -> ('bl, 'bv, 'ax) t

  (** removes the binding for this variable *)
  val ax_del : ('bl, 'bv, 'ax) t -> ax_var -> ('bl, 'bv, 'ax) t

  (** store a value for the value in this declaration *)
  val decl_store : ('a, 'a, 'a) t -> decl -> 'a -> ('a, 'a, 'a) t

  (** removes the binding for the variable declared in this declaration *)
  val decl_del : ('bl, 'bv, 'ax) t -> decl -> ('bl, 'bv, 'ax) t
end
