...
struct
  ...
  type t = {
    bl_bind : (bl_var, bl_term) status BlVarMap.t;
    bv_bind : (bv_var, bv_term) status BvVarMap.t;
    ax_bind : (ax_var, ax_term) status AxVarMap.t;
  }
  ...
  let bl_lookup t v = try BlVarMap.find v t.bl_bind with Not_found -> Free
  let bv_lookup t v = try BvVarMap.find v t.bv_bind with Not_found -> Free
  let ax_lookup t v = try AxVarMap.find v t.ax_bind with Not_found -> Free

  let bl_decl t v ls = {t with bl_bind = BlVarMap.add v (Declared (v,ls)) t.bl_bind;}
  let bv_decl t v ls = {t with bv_bind = BvVarMap.add v (Declared (v,ls)) t.bv_bind;}
  let ax_decl t v ls = {t with ax_bind = AxVarMap.add v (Declared (v,ls)) t.ax_bind;}
  ...
  let bl_def t v ls bl = {t with bl_bind = BlVarMap.add v (Defined (v,ls,bl)) t.bl_bind;}
  let bv_def t v ls bv = {t with bv_bind = BvVarMap.add v (Defined (v,ls,bv)) t.bv_bind;}
  let ax_def t v ls ax = {t with ax_bind = AxVarMap.add v (Defined (v,ls,ax)) t.ax_bind;}

  let rec def is_var add find v ls tm t: t =
    match is_var tm with
    | None -> add v ls tm
    | Some v' ->
      if v <> v' then
        match find v' with
        | Defined (_,[],tm) -> def is_var add find v ls tm t
        | _ -> add v ls tm
      else t

  let def t df =
    match df.def_desc with
    | BlDef (v,ls,bl) -> def is_bl_var (bl_def t) (bl_lookup t) v ls bl t
    | BvDef (v,ls,bv) -> def is_bv_var (bv_def t) (bv_lookup t) v ls bv t
    | AxDef (v,ls,ax) -> def is_ax_var (ax_def t) (ax_lookup t) v ls ax t

  let undecl t dc =
    match dc.decl_desc with
    | BlDecl (v,_) -> {t with bl_bind = BlVarMap.remove v t.bl_bind;}
    | BvDecl (v,_) -> {t with bv_bind = BvVarMap.remove v t.bv_bind;}
    | AxDecl (v,_) -> {t with ax_bind = AxVarMap.remove v t.ax_bind;}

  let undef t df =
    match df.def_desc with
    | BlDef (v,_,_) -> {t with bl_bind = BlVarMap.remove v t.bl_bind;}
    | BvDef (v,_,_) -> {t with bv_bind = BvVarMap.remove v t.bv_bind;}
    | AxDef (v,_,_) -> {t with ax_bind = AxVarMap.remove v t.ax_bind;}
