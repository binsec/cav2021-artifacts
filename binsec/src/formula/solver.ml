...
struct
  open Smtlib
  let put_entry en = Formula_to_smtlib.entry en
  let check_sat = { command_desc = CmdCheckSat; command_loc = Location.dummy_loc }
  let get_model = { command_desc = CmdGetModel; command_loc = Location.dummy_loc }
  let get_value v = 
    let term = Formula_to_smtlib.term v in
    { command_desc = CmdGetValue [term]; command_loc = Location.dummy_loc }
end
...
let stop_interactive session =
  ignore @@
    Subprocess.close_process (session.stdin, session.stdout, session.stderr);
...
let get_result_and session f =
  ...
  with End_of_file ->
    ...
    begin
      try
        while true do
          let stderr = input_line session.stderr in
          Logger.error "Stderr: %s" stderr;
        done
      with End_of_file -> () | e -> Logger.error "While retrieving stderr: %s" (Printexc.to_string e)
    end;
...
let solve_model_time ?(timeout=0) ?(get_model=false) ~file solver =
  ...
  let fdin, fdout, fderr = Subprocess.open_process args in
  ...
  Subprocess.close_process (fdin, fdout, fderr) |> ignore;
...
let start ?(interactive=true) ?file ?(timeout=0) solver =
  let incremental = interactive in
  let fullcmdline = Prover.command ~incremental timeout solver in
  let fullcmdline = if incremental then fullcmdline else Array.concat [
      fullcmdline; [| "/dev/stdin" |] 
    ]
  in
  let fdin, fdout, fderr =
    Subprocess.open_process fullcmdline in
...
module Session = struct
  ...
  let create ?(interactive=true) ?file ?(timeout=0) solver = {
    process = start ~interactive ?file ~timeout solver;
    state = None;
    model = None;
    file;
  }
  ...
  let run session command =
    ...
    match command.Smtlib.command_desc with
    | Smtlib.CmdCheckSat ->
