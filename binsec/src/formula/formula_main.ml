...
let transform ~filename =
  ...
  in
  let fm = Smtlib_to_formula.script smt_script in
  (* itv & keep are set to their default values *)
  let opt = Formula_transformation.optimize
    ~keep:Formula.VarSet.empty ?lst ~cst ~itv ~prn ~rbs ~row ~ssa ~ai fm
  in
  Formula_to_smtlib.formula opt,
  match lst with
