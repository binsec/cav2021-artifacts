...
let formula ?(logic=Some("QF_ABV")) ast =
  let open Smtlib in
  let script_commands = visit_formula () ast |> list_of_sequence in
  let script_commands = match logic with
    | Some x -> (mk_command (CmdSetLogic (mk_symbol x)))::script_commands 
    | None -> script_commands
  in
