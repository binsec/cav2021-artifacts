...
module Command :
sig
  val check_sat : Smtlib.command
  val get_model : Smtlib.command
  val get_value : Formula.term -> Smtlib.command
  val put_entry : Formula.entry -> Smtlib.command
end
...
(** [start ~file ~timeout solver] starts an interactive session
    ...
    are also copied. 
    If interactive is false, then interactivity is simulated but (check-sat) may only
    be submitted once.
*)
val start :
  ?interactive:bool ->
...
module Session : sig
  ...
  val create : ?interactive:bool -> ?file:string -> ?timeout:int -> Prover.t -> t

  val file : t -> string option

  val destroy : t -> unit

  val run : t -> Smtlib.command -> output
