...
type 'a t =
  ...
  | Nondet of Dba.LValue.t * Dba.Non_deterministic_tag.t
  | Stop of Dba.state
...
let non_deterministic ?tag lval =
  let tag =
    match tag with
    | None -> Dba.Non_deterministic_tag.create ()
    | Some t -> t in
  Nondet (lval, tag)
;;
...
let to_dba_instruction next_id = function
  ...
  | Nondet (lhs, tag) -> Dba.Instr.non_deterministic lhs ~tag next_id
