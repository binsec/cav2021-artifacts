...
let checked_assign lval rval id =
  let lval_sz = Dba.LValue.bitsize lval |> Size.Bit.to_int
  and rval_sz = Dba.Expr.size_of rval in
  if lval_sz != rval_sz then
    Logger.fatal ~e:Parsing.Parse_error"Cannot assign %d-bit rvalue %a to %d-bit lvalue %a"
      rval_sz
      Dba_printer.EICUnicode.pp_expr rval
      lval_sz
      Dba_printer.EICUnicode.pp_expr (Dba.LValue.to_expr lval)
  else
    Dba.Instr.assign lval rval id
...
module Initialization = struct
  ...
  type operation =
    ...
    | Declaration  of Dba.LValue.t
    | Instruction of Dba.Instr.t

  let pp_operation ppf op = match (op : operation) with
      | Assignment (lval, _, _) -> Format.fprintf ppf "%a := ???;" Dba_printer.Ascii.pp_lhs lval
      | Mem_load (bv, _) -> Format.fprintf ppf "@[%a] from_file;" Bitvector.pp bv
      | Declaration lval -> Format.fprintf ppf "declare %a;" Dba_printer.Ascii.pp_lhs lval
      | Instruction instr -> Dba_printer.Ascii.pp_instruction ppf instr
  ...
  let pp ppf t = Format.fprintf ppf "%a %a"
      Dba.Non_deterministic_tag.pp_control t.controlled
      pp_operation t.operation

  let create ~controlled ~operation = { controlled; operation; }

  let assign ?identifier ~controlled lval rval =
    let operation = Assignment (lval, rval, identifier) in
    create ~controlled ~operation

  let declaration ~controlled lval =
    create ~controlled ~operation:(Declaration lval)

  let from_assignment ~controlled = function
    | Dba.Instr.Assign(lval, rval, _) -> assign ~controlled lval (Singleton rval)
    | _ -> failwith "initialization with non assignment"

  let from_instruction instr = 
    let operation = Instruction instr in
    create ~controlled:Dba.Non_deterministic_tag.Uncontrolled ~operation

  let from_store ~controlled lv =
  ...
  let mark_more_controlled controlled t =
    if t.controlled = Dba.Non_deterministic_tag.Uncontrolled then
      { t with controlled }
    else if t.controlled <> controlled then
      Logger.fatal "Cannot modify control of %a from %a to %a"
        pp_operation t.operation
        Dba.Non_deterministic_tag.pp_control t.controlled
        Dba.Non_deterministic_tag.pp_control controlled
    else t
end
