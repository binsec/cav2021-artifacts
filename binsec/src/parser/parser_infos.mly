  ...
  let rec resolve_labels insns =
    ...
    | i :: l ->
      let h =
        ...
        | Nondet (a, tag, c) -> Instr.non_deterministic a ~tag c
      ...
      | i :: [] -> (
        ...
        | Dba.Instr.NondetAssume (a, b, _) ->
           l @ [addr, Instr.non_deterministic_assume a b (addr.id + 1);
                incindex addr 1, last_instr]
        | Dba.Instr.Nondet (a, tag, _) ->
           l @ [addr, Instr.non_deterministic a ~tag (addr.id + 1);
                incindex addr 1, last_instr]
        | Dba.Instr.Print (a, _) ->
...
regionnondet :
 | CONSTANT { Dba.Non_deterministic_tag.create ~region:`Constant () }
 | STACK    { Dba.Non_deterministic_tag.create ~region:`Stack () }
 | MALLOC
     { Dba.Non_deterministic_tag.create ~region:
