...
%token ENUMERATE REACH CUT CONSEQUENT ALTERNATIVE ALTERNATE CONTROLLED
%token FORALL
...
control_tag:
  | CONTROLLED { Dba.Non_deterministic_tag.Controlled }
  | FORALL { Dba.Non_deterministic_tag.Parameter }
rvalue:
| e=expr; { fun lv -> checked_assign lv e }
| UNDEF   { Dba.Instr.undefined }
| MALLOC e=delimited(LPAR, expr, RPAR);
  { fun lv -> Dba.Instr.malloc lv e  }
| NONDET
  region=ioption(delimited(LPAR,regionnondet, RPAR));
  name=ioption(as_annotation);
  { let tag =
      Dba.Non_deterministic_tag.create ?name ~controlled:Dba.Non_deterministic_tag.Uncontrolled ?region () in
    fun lv -> Dba.Instr.non_deterministic lv ~tag }
| ctag=control_tag name=ioption(as_annotation);
  { fun lv ->
    let tag = Dba.Non_deterministic_tag.create ?name ~controlled:ctag () in
    Dba.Instr.non_deterministic ~tag lv }
;
...
| a=assignment;              { Initialization.from_assignment  ~controlled:Dba.Non_deterministic_tag.Uncontrolled a }
| v=address_lvalue; fromfile { Initialization.from_store ~controlled:Dba.Non_deterministic_tag.Uncontrolled v }
| lvalue=lvalue; ASSIGN; is=interval_or_set; idopt=ioption(as_annotation);
  { Initialization.assign ~controlled:Dba.Non_deterministic_tag.Uncontrolled ?identifier:idopt lvalue is }
| lv=lvalue;  { Initialization.declaration ~controlled:Dba.Non_deterministic_tag.Uncontrolled lv }
;

initialization_directive:
| controlled=control_tag; init=initialization_assignment;
  { Initialization.mark_more_controlled controlled init }
| init=initialization_assignment;
  { init }
| instr=statement_instruction;
  { Initialization.from_instruction instr }
;
...
| statement=statement_instruction; { statement }
...
reach_cond:
  | oe = option(IF e=expr{e}) { oe }
;


/* Directives for analyses: SE, interpretation  */
directive:
 | REACH times=integer_argument cond=reach_cond;
   { let n = Utils.get_opt_or_default 1 times in Directive.reach ~n ?cond }
 | REACH STAR cond=reach_cond;
   { Directive.reach_all ?cond }
