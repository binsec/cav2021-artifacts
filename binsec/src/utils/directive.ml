...
type d =
  | Reach of Count.t * Dba.Expr.t option
...
let pp_goal ppf = function
  | Reach (c, _cond) -> fprintf ppf "reach %a" Count.pp c
  | Enumerate (c, _e) ->
...
let reach ?(n=1) ?cond loc =
  assert (n >= 0);
  { loc; goal = Reach (Count.count n, cond); }

let reach_all ?cond loc = { loc; goal = Reach (Count.unlimited, cond); }
...
let check_and_decr g =
  ...
  | Assume _
  | Cut
  | Reach (Count.Unlimited, _) | Reach (Count.Count 0, _)
